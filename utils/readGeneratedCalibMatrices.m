function [cMat,secMat,WorkbenchMat,extraCoeff]=readGeneratedCalibMatrices(experimentNames,scriptOptions,sensorsToAnalize,names2use,lambdasNames)
...
for i=1:length(experimentNames)
    ...
    for lam=1:length(lambdasNames)
        ...
        for j=1:length(sensorsToAnalize)
            ...
            secMat.(names2use{(i-1)*length(lambdasNames)+1+lam}).(sensorsToAnalize{j})= cMat.(names2use{(i-1)*length(lambdasNames)+1+lam}).(sensorsToAnalize{j})(1:6,1:6)/WorkbenchMat.(sensorsToAnalize{j});
            if size(cMat.(names2use{(i-1)*length(lambdasNames)+1+lam}).(sensorsToAnalize{j}),2)>6
                extraCoeff.(names2use{(i-1)*length(lambdasNames)+1+lam}).(sensorsToAnalize{j})=cMat.(names2use{(i-1)*length(lambdasNames)+1+lam}).(sensorsToAnalize{j})(:,7:end);
                cMat.(names2use{(i-1)*length(lambdasNames)+1+lam}).(sensorsToAnalize{j})=cMat.(names2use{(i-1)*length(lambdasNames)+1+lam}).(sensorsToAnalize{j})(1:6,1:6);
            else
                extraCoeff.(names2use{(i-1)*length(lambdasNames)+1+lam}).(sensorsToAnalize{j})=zeros(6,1);                
            end
        end
    end
end
