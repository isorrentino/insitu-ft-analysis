...
function [reCalibData,offsetInWrenchSpace,varargout]=calculateMatrixPerformance(datasetToUse,sensorsToAnalize,calibMatrices,offset,checkMatrixOptions,varargin)
...
if (~any(strcmp('otherCoeffFirstValAsOffset', optionsFieldNames)))
    checkMatrixOptions.otherCoeffValAsOffset=0;
...
for ftIdx =1:length(sensorsToAnalize)
    ...
    if ~isempty(otherCoeff)
        ...
        if checkMatrixOptions.otherCoeffValAsOffset~=0
        recabInput{recabnarin+1}='varOff';            
        recabInput{recabnarin+2}=checkMatrixOptions.otherCoeffFirstValAsOffset;
        end
    end
